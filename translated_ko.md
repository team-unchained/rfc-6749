...
이 사양은 OAuth 액세스 토큰 유형 등록소를 설정합니다.
...
IANA는 지정된 전문가로부터 직접 받은 요청만 등록소에 반영해야 하며, 모든 등록 요청을 검토 메일링 리스트로 안내해야 합니다.
...
- 추가 토큰 엔드포인트 응답 매개변수:
  ...
  새로운 매개변수는 반드시 OAuth 매개변수 등록소에 별도로 등록해야 합니다(MUST). 등록 절차는 문서의 11.2절에서 설명하고 있습니다.
- HTTP 인증 방식:
...
OAuth 매개변수 등록소의 초기 내용은 다음과 같습니다.
- 매개변수 이름: `client_id`
...
이 사양은 OAuth 인가 엔드포인트 응답 유형 등록소를 설정합니다.
...
IANA는 지정된 전문가로부터 받은 요청만 등록소에 반영해야 합니다. 
...
OAuth 인가 엔드포인트 응답 유형 등록소의 초기 내용은 다음과 같습니다.
- 응답 유형 이름: `code`
- 변경 관리 주체: IETF
- 사양 문서: RFC 6749

...
- scope: 
  ...
  액세스 토큰의 범위는 [섹션 3.3](#33-액세스-토큰-범위)에서 설명된 대로 설정됩니다.
- state: 
...
인가 엔드포인트는 클라이언트와 리소스 소유자의 사용자 에이전트 간의 상호작용을 요구합니다. 네이티브 애플리케이션은 외부 사용자 에이전트를 호출하거나 애플리케이션 내에 사용자 에이전트를 포함할 수 있습니다. 예를 들어:

- 외부 사용자 에이전트 - 네이티브 애플리케이션은 운영 체제에 등록된 스킴을 사용하여 클라이언트를 핸들러로 호출하는 리디렉션 URI를 통해 인가 서버의 응답을 캡처하거나, 자격 증명을 수동으로 복사하여 붙여넣기, 로컬 웹 서버 실행, 사용자 에이전트 확장 설치, 또는 클라이언트가 제어하는 서버 호스팅 리소스를 식별하는 리디렉션 URI를 제공하여 네이티브 애플리케이션에 응답을 제공할 수 있습니다.

- 내장된 사용자 에이전트 - 네이티브 애플리케이션은 리소스 로드 중에 발생하는 상태 변화를 모니터링하거나 사용자 에이전트의 쿠키 저장소에 접근하여 내장된 사용자 에이전트와 직접 통신하여 응답을 얻습니다.

외부 또는 내장된 사용자 에이전트 중에서 선택할 때, 개발자는 다음을 고려해야 합니다:

- 외부 사용자 에이전트는 리소스 소유자가 이미 인가 서버와 활성 세션을 가지고 있을 수 있어 재인증이 필요 없으므로 완료율을 향상시킬 수 있습니다. 이는 익숙한 최종 사용자 경험과 기능을 제공합니다. 리소스 소유자는 또한 인증을 돕기 위해 사용자 에이전트 기능이나 확장(예: 비밀번호 관리자, 2단계 인증 장치 리더기)에 의존할 수 있습니다.

- 내장된 사용자 에이전트는 컨텍스트 전환과 새 창을 열 필요가 없어 사용성을 향상시킬 수 있습니다.

- 내장된 사용자 에이전트는 보안 문제를 제기합니다. 왜냐하면 리소스 소유자가 대부분의 외부 사용자 에이전트에서 발견되는 시각적 보호 없이 식별되지 않은 창에서 인증을 수행하기 때문입니다. 내장된 사용자 에이전트는 최종 사용자에게 식별되지 않은 인증 요청을 신뢰하도록 교육하여 피싱 공격을 쉽게 실행할 수 있게 만듭니다.
...
이번 절에서는 구현자에게 2.1절에서 설명한 세가지의 클라이언트 프로필인 웹 애플리케이션, 사용자 에이전트 기반 애플리케이션, 네이티브 애플리케이션에 중점을 둔 보안 가이드라인을 제공합니다.
...
인가 서버는 클라이언트 인증을 목적으로 네이티브 애플리케이션이나 사용자 에이전트 기반 애플리케이션에게 클라이언트 비밀번호를 비롯한
...
인가 코드는 사용자 에이전트 리디렉션을 통해 전송되므로 사용자 에이전트 기록 및 HTTP Referrer 헤더를 통해 노출될 수 있습니다.
...
공격자가 리디렉션 URI의 값을 조작할 수 있다면, 인가 서버가 리소스 소유자의 사용자 에이전트로 하여금
공격자가 제어하는 URI로 인가 코드를 포함하여 리디렉션하도록 할 수 있습니다.

공격자는 합법적인 클라이언트에서 계정을 생성한 후, OAuth 인증 흐름을 시작할 수 있습니다.
공격자의 사용자 에이전트가 인가 서버에 접근하여 인증을 요청하면, 클라이언트는 인가 코드를 받기 위해 특정 인가 URI를 생성합니다.
...
클라이언트 개발자는 사용자 에이전트(예: 외부, 내장형)와의 상호작용 방식이 보안에 미치는 영향을 고려해야 하며, 사용자가 인가 서버의 진위를 확인할 수 있는 능력을 갖추도록 해야 합니다.

피싱 공격의 위험을 줄이기 위해서 인가 서버는 반드시 사용자와 상호작용하는 모든 엔드포인트가 TLS 를 사용하도록 요구해야 합니다 (MUST).

### 10.12 크로스 사이트 요청 위조 (CSRF)

교차 사이트 요청 위조는 공격자가 피해자의 사용자 에이전트를 조작하여 악성 URI(예: 사용자를 속이는 링크, 이미지 또는 리디렉션 형태로 제공됨)를 신뢰하는 서버(일반적으로 유효한 세션 쿠키가 존재하는 서버)로 요청하도록 유도하는 공격 기법입니다.
...
이는 일반적으로 리디렉션 URI 엔드포인트로 전송되는 요청에 대해 사용자 에이전트의 인증된 상태와 요청을 연결하는 값을 포함하도록 요구하는 방식으로 이루어집니다. (예: 사용자 에이전트를 인증하는 세션 쿠키의 해시 값)
...
이 값을 이용해 클라이언트는 사용자 에이전트의 인증 상태와 비교함으로써 요청의 유효성을 검증할 수 있습니다.
CSRF 보호를 위해 사용되는 바인딩 값은 추측할 수 없는 값이어야 하며(10.10절 참고), 사용자 에이전트의 인증 상태(예: 세션 쿠키, HTML5 로컬 스토리지)는 클라이언트와 사용자 에이전트만 접근할 수 있는 위치(즉, same-origin 정책에 의해 보호되는 위치)에 저장되어야 합니다.
...
암묵적 흐름(response_type=token)에서는 공격자가 인가 서버의 응답에 있는 토큰을 쉽게 바꿔치기할 수 있으며, 실제 액세스 토큰을 이전에 발급받은 자신의 토큰으로 교체할 수 있습니다.
...
### 11.1. OAuth 액세스 토큰 유형 등록

이 사양은 OAuth 액세스 토큰 유형 레지스트리를 설정합니다.

액세스 토큰 유형은 반드시 Specification Required 절차에 따라 등록해야 합니다([RFC5226](https://datatracker.ietf.org/doc/html/rfc5226)).
등록 전 oauth-ext-review@ietf.org 메일링 리스트에서 2주간의 검토 기간을 거친 후, 한 명 이상의 지정된 전문가(Designated Expert)의 조언을 기반으로 승인됩니다. 
그러나, 해당 사양이 공개될 예정이라는 점이 확인될 경우, 지정된 전문가가 등록을 사전 승인할 수도 있습니다.

등록 요청은 검토 및 의견 수렴을 위해 oauth-ext-review@ietf.org 메일링 리스트로 보내야 하며, 적절한 제목(예: “Request for access token type: example”)을 포함해야 합니다.

검토 기간 내에 지정된 전문가들은 등록 요청을 승인하거나 거부하며, 그 결정을 검토 리스트 및 IANA에 전달해야 합니다. 
거부하는 경우, 거절 사유와 함께 요청을 성공적으로 변경할 수 있는 제안을 포함해야 합니다.

IANA는 지정된 전문가로부터 직접 받은 요청만 레지스트리에 반영해야 하며, 모든 등록 요청을 검토 메일링 리스트로 안내해야 합니다.

#### 11.1.1. 등록 템플릿

- 유형 이름:

  요청된 이름을 기재합니다(예: “example”).
- 추가 토큰 엔드포인트 응답 매개변수:

  `access_token` 매개변수와 함께 반환되는 추가 응답 매개변수를 기재합니다.
  새로운 매개변수는 반드시 OAuth 매개변수 레지스트리에 별도로 등록해야 합니다(MUST). 등록 절차는 문서의 11.2절에서 설명하고 있습니다.
- HTTP 인증 방식:

- 변경 관리 주체:
  이 유형의 액세스 토큰을 사용하여 보호된 리소스 요청을 인증하는 HTTP 인증 방식이 있다면, 해당 방식을 기재해야 합니다.

  Standards Track RFC의 경우 “IETF”로 명시해야 합니다.
  그 외의 경우, 책임 기관의 이름을 기재하며, 필요에 따라 우편 주소, 이메일 주소, 홈페이지 URI 등의 세부 정보를 포함할 수 있습니다.
- 사양 문서:

  해당 매개변수를 규정하는 문서를 참조해야 합니다.
  가능하면 문서 사본을 가져올 수 있는 URI를 포함하는 것이 좋습니다.
  또한, 관련된 섹션을 명시할 수도 있지만, 반드시 포함해야 하는 것은 아닙니다.

### 11.2. OAuth 매개변수 등록
이 사양은 OAuth 매개변수 레지스트리를 설정합니다.

인가 엔드포인트 요청, 인가 엔드포인트 응답, 토큰 엔드포인트 요청 또는 토큰 엔드포인트 응답에 포함될 추가 매개변수는 반드시 Specification Required 절차에 따라 등록해야 합니다([RFC5226](https://datatracker.ietf.org/doc/html/rfc5226)). 
등록 전, oauth-ext-review@ietf.org 메일링 리스트에서 2주간의 검토 기간을 거쳐야 하며, 이 과정에서 한 명 이상의 지정된 전문가의 조언을 받게 됩니다. 
하지만, 공식적으로 문서가 공개되기 전에 값을 할당할 수 있도록 하기 위해, 지정된 전문가가 해당 사양이 공개될 것이라고 판단하면 등록을 사전 승인할 수 있습니다.

등록 요청은 검토 및 의견 수렴을 위해 oauth-ext-review@ietf.org 메일링 리스트로 제출해야 합니다. 
또한, 적절한 제목을 포함해야 합니다(예: “Request for parameter: example”).

검토 기간 동안, 지정된 전문가는 등록 요청을 승인하거나 거부하게 되며, 그 결정은 검토 리스트와 IANA에 전달됩니다. 
거부하는 경우, 그 이유를 설명해야 하며, 필요한 경우 요청을 성공적으로 변경할 수 있는 제안을 포함해야 합니다.

IANA는 지정된 전문가로부터 받은 등록 요청만 수락해야 합니다. 또한, 모든 등록 요청을 검토 메일링 리스트로 안내하는 것이 바람직합니다.
...
- 매개변수 사용 위치:
  ...
  해당 매개변수를 사용할 수 있는 위치를 나타냅니다.
  가능한 위치로는 인가 요청, 인가 응답, 토큰 요청 또는 토큰 응답이 있습니다.
- 변경 관리 주체:

  Standards Track RFC의 경우 “IETF”로 명시해야 합니다.
  그 외의 경우, 책임 기관의 이름을 기재해야 합니다.
  필요에 따라 우편 주소, 이메일 주소, 홈페이지 URI 등의 세부 정보를 포함할 수 있습니다.
...
- 매개변수 이름: `scope`
- 매개변수 사용 위치: 인가 요청, 인가 응답, 토큰 요청, 토큰 응답
- 변경 관리 주체: IETF
- 사양 문서: RFC 6749


- 매개변수 이름: `state`
- 매개변수 사용 위치: 인가 요청, 인가 응답
- 변경 관리 주체: IETF
- 사양 문서: RFC 6749


- 매개변수 이름: `code`
- 매개변수 사용 위치: 인가 응답, 토큰 요청
- 변경 관리 주체: IETF
- 사양 문서: RFC 6749


- 매개변수 이름: `error_description`
- 매개변수 사용 위치: 인가 응답, 토큰 응답
- 변경 관리 주체: IETF
- 사양 문서: RFC 6749
...
- 매개변수 사용 위치: 토큰 요청, 토큰 응답
- 변경 관리 주체: IETF
- 사양 문서: RFC 6749

### 11.3. OAuth 인가 엔드포인트 응답 유형 등록


이 사양은 OAuth 인가 엔드포인트 응답 유형 레지스트리를 설정합니다.

인가 엔드포인트에서 사용할 추가 응답 유형은 Specification Required 절차에 따라 등록해야 합니다.([RFC5226](https://datatracker.ietf.org/doc/html/rfc5226)).
등록 전에 oauth-ext-review@ietf.org 메일링 리스트에서 2주간의 검토 기간을 거쳐야 하며, 이 과정에서 한 명 이상의 지정된 전문가의 조언을 받게 됩니다.

그러나 공식 문서가 공개되기 전에 값을 할당할 수 있도록 하기 위해, 지정된 전문가는 해당 사양이 공개될 것이라고 판단되면 등록을 승인할 수 있습니다.

등록 요청은 검토 및 의견 수렴을 위해 oauth-ext-review@ietf.org 메일링 리스트로 제출해야 합니다. 
또한, 적절한 제목을 포함해야 합니다(예: “Request for response type: example”).

검토 기간 동안, 지정된 전문가는 등록 요청을 승인하거나 거부합니다. 
승인 또는 거부 결정은 검토 리스트와 IANA에 전달됩니다. 
거부하는 경우, 그 이유를 설명하고, 필요한 경우 요청을 성공적으로 변경할 수 있는 제안을 포함해야 합니다.

IANA는 지정된 전문가로부터 받은 요청만 레지스트리에 반영해야 합니다. 
또한, 모든 등록 요청을 검토 메일링 리스트로 안내해야 합니다.
...
- 사양 문서:
  ...
  해당 유형을 규정하는 문서를 참조해야 합니다.
  가능하면 문서 사본을 가져올 수 있는 URI를 포함하는 것이 좋습니다.
  또한, 관련된 섹션을 명시할 수도 있지만, 반드시 포함해야 하는 것은 아닙니다.

#### 11.3.2. 초기 등록 내용

OAuth 인가 엔드포인트 응답 유형 레지스트리의 초기 내용은 다음과 같습니다.
- 응답 유형 이름: `code`
- 변경 관리 주체: IETF
- 사양 문서: RFC 6749
...
기타 프로토콜 확장(예: 확장 보조 유형, 액세스 토큰 유형 또는 확장 매개변수)과 함께 사용되는 추가 오류 코드는 Specification Required 절차에 따라 등록됩니다.([RFC5226](https://datatracker.ietf.org/doc/html/rfc5226)).
등록 전에 oauth-ext-review@ietf.org 메일링 리스트에서 2주간의 검토 기간을 거쳐야 하며, 이 과정에서 한 명 이상의 지정된 전문가의 조언을 받게 됩니다.

그러나 공식 문서가 공개되기 전에 값을 할당할 수 있도록 하기 위해, 지정된 전문가는 해당 사양이 공개될 것이라고 판단되면 등록을 승인할 수 있습니다.

등록 요청은 검토 및 의견 수렴을 위해 oauth-ext-review@ietf.org 메일링 리스트로 제출해야 합니다. 
또한, 적절한 제목을 포함해야 합니다(예: “Request for error code: example”).
...
- 오류 이름:
  ...
  요청된 오류 이름을 기재합니다(예: “example”).
  오류 이름에는 %x20-21, %x23-5B, %x5D-7E 범위를 벗어나는 문자를 포함해서는 안 됩니다(MUST NOT).
- 오류 사용 위치:

  오류가 사용될 수 있는 위치를 나타냅니다.
  가능한 위치로는 인가 코드 부여 오류 응답([4.1.2.1절](#4121-오류-응답)), 암시적 부여 오류 응답([4.2.2.1절](#4221-오류-응답)), 토큰 오류 응답([5.2절](#52-오류-응답)), 또는 리소스 액세스 오류 응답([7.2절](#72-오류-응답))이 있습니다.
- 관련 프로토콜 확장:

  오류 코드가 함께 사용되는 확장 보조 유형의 이름, 액세스 토큰 유형, 또는 확장 매개변수의 이름을 기재합니다.
- 변경 관리 주체:
